...
local trsize = 96300 -- 11568
local tesize = 65900  -- 5272
...
for set = 1, 2 do
    ...
    local trainData = torch.load('./images_background/images_background/augmented_omniglot/trainData/trainData_' .. set .. '.t7')

    print ("loading testing Data .....")
    local testData = torch.load("./images_evaluation/images_evaluation/augmented_omniglot/testData/testData_" .. set .. ".t7")
...
--[[
  ...
  -- preprocess testSet
    ...
    local mean_u = trainData.data:mean() 
    local std_u = trainData.data:std()
    ...
    trainData.data =  torch.reshape(trainData.data, trsize, 1, 1, 128, 128) 
    testData.data =  torch.reshape(testData.data, tesize, 1, 1, 128, 128) 
...
for b_train = (set-1)*100000/opt.batchSize + 1, (set)*100000/opt.batchSize do

    local catTrainData = {
        data = torch.CudaTensor(opt.batchSize, 2, 1, 128 , 128):fill(0),
    ...
    torch.save('./omniglot_10xAug/train/' .. b_train .. '.t7', catTrainData)
...
for b_test = (set-1)*50000/opt.batchSize + 1 , (set)*50000/opt.batchSize do
    ...
    torch.save('./omniglot_10xAug/test/' .. b_test .. '.t7', catTestData) 
end
