...
imgPaths, imgLabels =  loadList('testing_paths.txt')
 ...
 max_index_tr = #imgLabels
    if max_index_tr % 500 ~=0 then
        max_index_tr = max_index_tr - (max_index_tr % 500)
    end 

print ("creating tensor ... ")

local trainData = {
        data = torch.Tensor(max_index_tr, 3, 128, 128),
        labels = torch.Tensor(max_index_tr),
        size = function() return max_index_tr end
    }

for i=1, max_index_tr do
    trainData.data[i] = image.load(imgPaths[i], 3, 'float') 
...
for b = 1, 10 do
    ...
    local batchData = {
        data = torch.Tensor(max_index_tr/10, 3, 128, 128),
        labels = torch.Tensor(max_index_tr/10),
        size = function() return max_index_tr/10 end 
    }   
    local count = 1

    for i= (b-1)*(max_index_tr/10)+1, b*max_index_tr/10 do
