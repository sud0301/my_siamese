...
cmd:option('-iterations', 8000*10,'Number of iterations')

cmd:option('-saveFile','net_wash20_10_2308_fine_tune.t7','Location for logging') -- to load: p = torch.load('test.t7')

cmd:option('-model', 'vgg_bn_drop_v3', 'model name')
cmd:option('-modelFile', 'net_wash20_10_2308_net_fine_tune.t7', 'model file name')
...
if opt.mode == 'image' then
    ...
    --model = torch.load('net_wash20_10_2308_net.t7')
